#!/usr/bin/env bash
# Run SPRINTER on FIT208 DLP+ per-sample CHISEL RDR data.
#
...
OUTDIR="/srv/home/aste0033/projects/MPNST/scDNA/SPRINTER/DLPp"
...
MIN_NUM_CELLS=15   # minimum cells to constitute a clone
MIN_PROP_CELLS=0.1 # minimum fraction of total cells for a clone
...
for sample in "${SAMPLES[@]}"; do
    ...
    log "${sample}: starting SPRINTER (${n_cells} cells)"
    ...
    sprinter "$input_gz" \
        ...
        --rtreads       "$RT_READS" \
        --cnreads       "$CN_READS" \
        --minnumcells   "$MIN_NUM_CELLS" \
        --minpropcells  "$MIN_PROP_CELLS" \
...
log "=== All DLP+ SPRINTER runs complete ==="
